...
async function createBookingCard(booking) {
    ...
    if (booking.additionalServices && booking.additionalServices.length > 0) {
        ...
        const paidServices = booking.additionalServices.filter(s => s.paid);
        ...
        if (paidServices.length > 0) {
            additionalServicesHTML += `
                <div class="services-list" style="margin-top: 10px;">
                    ...
                    ${paidServices.map(s => `
                        <div class="service-item" style="background: #d4edda; border-right: 3px solid #28a745;">
                            <span>${s.serviceName}</span>
                            <span>${s.price.toFixed(2)} جنيه</span>
                        </div>
                    `).join('')}
...
window.endSession = async function(bookingId) {
    try {
        ...
        if (unpaidServices.length > 0) {
            ...
            if (!confirm(`سيتم إنهاء الجلسة\n\nيوجد ${unpaidServices.length} خدمة إضافية غير مدفوعة بمبلغ ${totalUnpaid.toFixed(2)} جنيه\n\nسيتم إرسال تنبيه لموظف الاستقبال لاستلام الدفع من العميلة عند الخروج`)) {
                return;
            }
            ...
            const amountNeeded = totalUnpaid - currentBalance;
            
            const existingFinalAlertQuery = query(
                collection(db, "receptionAlerts"),
                where("bookingId", "==", bookingId),
                where("stage", "==", "final_payment"),
                where("status", "==", "pending")
            );
            const existingFinalAlerts = await getDocs(existingFinalAlertQuery);
            
            if (existingFinalAlerts.empty) {
                await addDoc(collection(db, "receptionAlerts"), {
                    ...
                    message: `انتهت جلسة ${booking.customerName} وتحتاج لدفع ${unpaidServices.length} خدمة إضافية`,
                    createdAt: Timestamp.now(),
                    createdBy: currentDoctorName
                });
            }
            
            alert(`✅ تم إنهاء الجلسة\n\n✅ تم إرسال تنبيه لموظف الاستقبال (${booking.createdBy})\n\nيرجى توجيه العميلة للدفع قبل المغادرة`);
            
        } else {
            if (!confirm('هل تريد إنهاء الجلسة؟\n\nسيتم حفظ التقرير في ملف الأدمن وملف العميل.')) return;
